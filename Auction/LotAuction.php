...
class LotAuction extends Table 
{
    ...
    protected function afterUpdate($id,$edit_type,$data) 
    {
        ...
        if($user_id == 0) {
            $sql = 'UPDATE `'.TABLE_PREFIX.'order` AS O '.
                   'JOIN `'.TABLE_PREFIX.'order_item` AS I ON(O.`auction_id` = "'.AUCTION_ID.'" AND O.`order_id` = I.`order_id`) '.
                   'SET I.`status` = "BID" '.
            ...
            $sql = 'UPDATE `'.TABLE_PREFIX.'order` AS O '.
                   'JOIN `'.TABLE_PREFIX.'order_item` AS I ON(O.`auction_id` = "'.AUCTION_ID.'" AND O.`user_id` = "'.$buyer['user_id'].'" AND O.`order_id` = I.`order_id`) '.
